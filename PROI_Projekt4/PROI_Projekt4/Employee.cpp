...
Employee::Employee(std::string firstName, std::string lastName, int employeeNr)
: name{Name(firstName, lastName)}, employeeNumber(employeeNr)
{
		//this->setEmployeeNumber();
		//this->setStationNumber(0);
}

bool Employee::operator==(const Employee& rhs) const noexcept
{
	return (this->getEmployeeNumber() == rhs.getEmployeeNumber());
}

bool Employee::operator<(const Employee& r) const noexcept
    {
